...
LightGBM ავირჩიეთ, რადგან Walmart-ის მონაცემები კლასიკური tabular ფორმატისაა და შეიცავს ერთმანეთისგან განსხვავებული ტიპის ბევრ feature-ს:
...
## რატომ LightGBM და არა CatBoost ან XGBoost

სამივე მოდელი gradient boosting-ის ოჯახს ეკუთვნის, მაგრამ განსხვავებული უპირატესობები აქვს.

CatBoost უფრო ბუნებრივად ამუშავებს კატეგორიულ ცვლადებს და ordered boosting-ის საშუალებით category-based leakage-ის რისკს ამცირებს. LightGBM-ში კატეგორიული feature-ების ტიპები ხელით უნდა განისაზღვროს — ჩვენს შემთხვევაში კატეგორიულ ნიშნებად გადავეცით `Store`, `Dept` და `Type_Enc`. LightGBM ავირჩიეთ, რადგან feature engineering-ის შემდეგ მონაცემების დიდი ნაწილი უკვე რიცხვითი იყო, ხოლო მოდელის სწრაფი სწავლება მრავალი ექსპერიმენტის ჩატარების საშუალებას იძლეოდა.

XGBoost ხშირად უფრო კონსერვატიულად ზრდის ხეებს და ზოგიერთ dataset-ზე უკეთ აკონტროლებს overfitting-ს. თუმცა Walmart-ის მოცულობის მონაცემებზე LightGBM-ის histogram-based სწავლება და მაღალი სიჩქარე პრაქტიკულად უფრო მოსახერხებელი აღმოჩნდა.

მნიშვნელოვანია, რომ **არც LightGBM, არც CatBoost და არც XGBoost** არ აგვარებს time-series leakage-ს ავტომატურად — ისტორიული გაყიდვების feature-ები ისე უნდა შეიქმნას, რომ prediction მომენტში მიუწვდომელი ინფორმაცია არ გამოიყენებოდეს.
...
**რატომ ამოვიღეთ `Year`, `CPI` და `Unemployment`.** Kaggle-ის test period შეიცავს 2013 წელს, 
რომელიც training მონაცემებში არ გვხვდებოდა — `Year` tree model-ს categorical-like split-ებით შეეძლო დაემუშავებინა და unseen 2013-ზე არასტაბილური 
extrapolation გაეკეთებინა. `CPI` და `Unemployment` test horizon-ის ნაწილისთვის თავდაპირველად მიუწვდომელია, ხოლო მათი 
...
LightGBM-ის correction ყოველთვის სრულად სანდო არ არის, ამიტომ validation-ზე შემოწმდა 25 თანაბრად დაშორებული \(\alpha \in [0,1.2]\): \(\alpha=0\) → საბოლოო პროგნოზი მთლიანად seasonal baseline; \(\alpha=1\) → LightGBM-ის correction სრულად გამოიყენება; \(0<\alpha<1\) → correction shrinkage-ით გამოიყენება. საბოლოოდ ირჩევა ის \(\alpha\), რომელიც validation WMAE-ს მინიმუმამდე ამცირებს. ვინაიდან `alpha=0` (სუფთა baseline) ყოველთვის candidate-ია, შერჩეული blend seasonal baseline-ზე უარესი ვერასდროს იქნება.
...
Random split არ გამოგვიყენებია. Training მონაცემების ბოლო **39 უნიკალური კვირა** მთლიანად გამოიყო validation block-ად — ეს ზუსტად ემთხვევა Kaggle test horizon-ის სიგრძეს.
...
ამით თავიდან ავიცილეთ random split leakage, overlapping validation windows-ის ცრუ ოპტიმისტური შედეგი, validation target-ების lag feature-ებში მოხვედრა და Kaggle-ისგან განსხვავებული მოკლე horizon.
...
მოდელის objective იყო `regression_l1`, ხოლო training sample weight — `train_weights = np.where(IsHoliday, 5.0, 1.0)`. Validation და early stopping-ის დროსაც holiday row-ებს წონა 5 ჰქონდათ, რადგან ჩვეულებრივი MAE-ს ოპტიმიზაციამ შეიძლება საშუალო კვირები კარგად იწინასწარმეტყველოს, მაგრამ Black Friday და Christmas spike-ებზე დიდი შეცდომა დაუშვას.
...
Private score Public score-ზე დაახლოებით **82.70 პუნქტით** და **2.80%-ით** უკეთესია. Leaderboard-ის ორ ნაწილს შორის ასეთი მცირე განსხვავება მიუთითებს, რომ fixed LightGBM pipeline შედარებით სტაბილურად generalize-დებოდა. დაახლოებით 25,000-იანი საწყისი Kaggle score არ გამოხატავდა LightGBM-ის რეალურ შესაძლებლობას — მთავარი პრობლემა submission row-order bug იყო, არა მოდელის არქიტექტურა. ამ შედეგით LightGBM-მა (Public WMAE 2958.05) N-BEATS-საც (3513.18) აჯობა, რაც ადასტურებს, რომ ძლიერი feature engineering, ზუსტი Lag-52 seasonal baseline და leakage-safe validation tree-based მოდელისთვის ამ dataset-ზე ძალიან ეფექტურია.

---

## Feature Importance

![Fixed LightGBM feature importance](lightgbm_feature_importance.png)
...
```text
LightGBM_Training
|-- LightGBM_Honest_Tuning
|   |-- leaves31_lr0.02_child50_l21.0
|   |-- leaves31_lr0.02_child50_l25.0
|   |-- ...
|   `-- leaves63_lr0.04_child100_l25.0
|-- LightGBM_Fixed_Final
`-- LightGBM_Fixed_ModelRegistration
```

MLflow-ში ინახება: hyperparameter candidate-ები, validation WMAE, best iteration, correction alpha, Lag-52 baseline WMAE, residual clipping საზღვრები, feature list, model artifact, history artifact, inference configuration და registered pyfunc pipeline. საბოლოო model registry name არის `WalmartLightGBMFixedPipeline`.
...
Submission-ის შექმნამდე უნდა შემოწმდეს:

```python
assert len(predictions) == len(test_raw)
assert np.isfinite(predictions).all()
assert test_ids_are_aligned
assert original_row_order_is_restored
```

Prediction row-order-ის შემოწმება აუცილებელია — სწორად გამოთვლილი პროგნოზებიც კი, არასწორ `Store`–`Dept`–`Date` row-ზე მოხვედრის შემთხვევაში, მთლიანად არასწორ Kaggle score-ს მოგვცემს.
...
```text
.
|-- model_experiment_LightGBM_fixed.ipynb
|-- features.py
|-- metrics.py
|-- mlflow_setup.py
|-- train.csv
...
|-- features.csv
|-- stores.csv
|-- lgbm_fixed_model.joblib
|-- lgbm_history.parquet
|-- lgbm_fixed_config.json
`-- submission_lightgbm_fixed.csv
```
...
```bash
pip install numpy pandas matplotlib scikit-learn lightgbm mlflow joblib
```

DagsHub-ის გამოყენების შემთხვევაში დამატებით: `pip install dagshub`.

---

## გაშვება

1. Kaggle-იდან ჩამოტვირთეთ მონაცემები;
2. `train.csv`, `test.csv`, `features.csv` და `stores.csv` მოათავსეთ პროექტის დირექტორიაში;
3. დარწმუნდით, რომ ხელმისაწვდომია `features.py`, `metrics.py` და `mlflow_setup.py`;
4. გახსენით `model_experiment_LightGBM_fixed.ipynb`;
5. გაუშვით ყველა cell თანმიმდევრობით;
6. შეამოწმეთ `baseline_wmae`, `best_score`, `best_params` და `best_alpha`;
7. ატვირთეთ `submission_lightgbm_fixed.csv` Kaggle-ზე;
8. Public და Private შედეგები დაამატეთ README-ის შედეგების ცხრილში.

**გამოყენებული ტექნოლოგიები:** Python, Pandas, NumPy, LightGBM, Scikit-learn, Matplotlib, MLflow, Joblib, DagsHub, Kaggle.
...
მნიშვნელოვნად არის დამოკიდებული სწორ feature engineering-სა და leakage-safe validation-ზე.
